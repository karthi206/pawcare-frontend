...
export default function CaseTracker() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (!casesRes.ok) {
        const body = await safeParseJson<{ error?: string; message?: string }>(casesRes);
        setError({
          ...
          message:
            ...
            body?.error ||
            (casesRes.status === 401
              ? "You need to log in to view your cases."
              : casesRes.status === 403
              ? "You don't have access to these cases."
              : `The server responded with an unexpected error (${casesRes.status}).`),
        });
        return;
      }
  ...
  if (error) {
    return (
      <ErrorState
        code={error.status ? `${error.status} Error` : "Network Error"}
        title={
          error.status === 401
            ? "Login Required"
            : error.status === 403
